...
if (~isnumeric(x1) || ~isvector(x1))
    error('intersecti:x1', 'X1 is not a numeric vector.')
end
if (~isnumeric(y1) || ~isvector(y1))
    error('intersecti:y1', 'Y1 is not a numeric vector.')
end
if (~isnumeric(x2) || ~isvector(x2))
    error('intersecti:x2', 'X2 is not a numeric vector.')
end
if (~isnumeric(y2) || ~isvector(y2))
    error('intersecti:y2', 'Y2 is not a numeric vector.')
end
...
if (nargout > 4)
    error('intersecti:nargout', ['Number of outputs (' num2str(nargout) ') is greater than 4.'])
end

% want (x1,y1) and (x2,y2) to all be row vectors
if ~isrow(x1)
    x1                      = x1';
end
if ~isrow(y1)
...
% NaN out any interpolated intersections that are outside the bounds of the segments' x/y ranges
