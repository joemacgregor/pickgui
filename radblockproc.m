...
% RADBLOCKPROC Consolidate and pre-process CReSIS ice-penetrating radar
% data files into manageable blocks.
% 
...
if (nargin < 7)
    error('radblockproc:inputs', 'Not enough input arguments.')
end
if ~ischar(dir_in)
...
if (~isnumeric(num_file_block) || (length(num_file_block) > 1))
...
if (~isnumeric(num_overlap) || (length(num_overlap) > 1))
...
for ii = 1:num_block
    ...
    for kk = curr_files
        ...
        [tmp1.twtt_bed, tmp1.data, tmp1.elev_air, tmp1.time, tmp1.lat, tmp1.lon, tmp1.twtt_surf, tmp1.twtt] ...
                            = deal(tmp1.Bottom, tmp1.Data, tmp1.Elevation, tmp1.GPS_time, tmp1.Latitude, tmp1.Longitude, tmp1.Surface, tmp1.Time);
        tmp1                = rmfield(tmp1, {'Bottom' 'Data' 'Elevation' 'GPS_time' 'Latitude' 'Longitude' 'Surface' 'Time'});
    ...
    % distribute block pieces
    block.lat               = [load_struct(:).lat];                         % latitude
    block.lon               = [load_struct(:).lon];                         % longitude
    ...
    block.elev_air          = [load_struct(:).elev_air];                    % aircraft elevation
    ...
    block.file_in           = {file_in_all(curr_files).name}';              % individual file names
    ...
    if (license('test', 'map_toolbox') && (block.lat(1) > 0)) % in Greenland and can do better x/y coordinates
        ...
        [block.x, block.y]  = ll2ps(block.lat, block.lon, lat_std);         % convert lat/lon to polar stereographic x/y (better for Antarctica then Greenland)
    end
    ...
    if isrow(block.twtt)
        block.twtt          = block.twtt';                                  % make sure traveltime is a column vector
    end
